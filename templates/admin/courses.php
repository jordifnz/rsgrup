  ...
  <tr>
    ...
    <td class="actions">
      <button class="btn btn-sm"
        onclick="openCourseModal(<?= htmlspecialchars(json_encode($c), ENT_QUOTES) ?>)">
        Editar
      </button>
      <form action="<?= BASE_URL ?>/admin/cursos/<?= $c['id'] ?>/eliminar"
...
<!-- Modal Curso (en el DOM directamente, nunca clonado) -->
  ...
  <div class="modal-box">
    ...
    <form action="<?= BASE_URL ?>/admin/cursos/guardar" method="POST" class="form">
      ...
      <div class="form-group">
        ...
        <textarea name="description" id="crs-desc" rows="6"></textarea>
      </div>
...
function openCourseModal(c) {
  ...
  // 2. Abrir modal (quita [hidden] → textarea visible en DOM)
  openModal('modal-course');

  // 3. Inicializar TinyMCE ahora que el elemento es visible
  initEditorInModal('crs-desc', descContent);
}
</script>
