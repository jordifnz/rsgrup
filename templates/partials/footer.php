...
// WhatsApp flotante
try {
    $waRow = Database::fetchRow("SELECT `value` FROM rsgrup_settings WHERE `key`='whatsapp_number'");
    $waNumber = $waRow['value'] ?? '';
} catch(\Throwable $e) { $waNumber = ''; }
?>
<?php if(!empty($waNumber)): ?>
<a href="https://wa.me/<?= htmlspecialchars(preg_replace('/[^0-9]/', '', $waNumber)) ?>"
